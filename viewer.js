...
{
	$('#refreshAll').click(function(){bgPage.CheckForUnreadStart();});
	$('#markAllRead').click(function(){MarkAllFeedsRead();});
	$('#refreshButton').click(function(){bgPage.CheckForUnreadStart(selectedFeedKey);});
	$('#markFeedReadButton').click(function(){MarkFeedRead(feeds[selectedFeedKey].id);});
	$('#showOptions').click(function(){chrome.tabs.create({url:'options.html'});});
	$('#addFeeds').click(function(){window.location = 'manage.html';});
});
...
function ShowFeed(key)
{
    ...
    $(li).click(function(){SelectFeed(key);focusFeed();UpdateTitle();return false;});
